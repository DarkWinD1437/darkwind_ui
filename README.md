...
- Animación de cierre cinematográfica (~6s, a la par de lo que dura el arranque): al cerrar la ventana (X, Alt+F4, el botón ⏻ o Ctrl+Shift+Q) se intercepta el cierre, se termina cada sesión de terminal activa (`pty_kill`), las columnas/terminal/explorador/teclado se apagan con el mismo fade reversado del arranque, el logo "DarkWinD_UI" aparece con el mismo efecto glitch/derez del arranque (esta vez arrancando ya distorsionado, con todo el overlay temblando) y colapsa, y un log de cierre en estilo terminal tipea línea por línea con tiempo de sobra para leerlo ("Cerrando entorno de desarrollo...", "Finalizando sesiones de terminal...", "Liberando conexiones activas...", "Hasta la próxima." con cursor parpadeante) antes de cerrar de verdad — el original cierra de forma instantánea con `app.quit()`, sin ningún paso visual. El botón ⏻ vive junto a los íconos de Settings/Shortcuts en el explorador de archivos porque en pantalla completa forzada no hay barra de título ni "X" nativa del sistema para cerrar
- Cursor de la terminal parpadeante (xterm.js no lo hace por defecto; el original sí) y un indicador animado de "Leyendo carpeta…" en el explorador mientras se lee un directorio — el original tenía un indicador equivalente (`space_bar_working`) que no se había portado hasta ahora
